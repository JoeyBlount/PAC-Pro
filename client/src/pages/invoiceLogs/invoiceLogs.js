...
const InvoiceLogs = () => {
  ...
  const API_URL = ((process.env.REACT_APP_BACKEND_URL || "https://pac-pro-506342087804.us-west2.run.app").replace(/\/+$/, "")) + "/api/pac"; // or your deployed backend URL
    ...
    try {
      ...
      recurringSnapshot.docs.forEach((docSnap) => {
        ...
        if (invDate >= currentDate) {
          monthsToUpdate.add(
            `${invData.storeID}|${invData.targetMonth}|${invData.targetYear}`
          );
        }
      });
      ...
      for (const monthKey of monthsToUpdate) {
        ...
        try {
          ...
          console.log(
            `Invoice totals updated for ${storeID} - ${targetMonth}/${targetYear}`
          );
        } catch (totalsError) {
  ...
  const handleRestore = async (invoice) => {
    ...
    try {
      ...
      try {
        ...
        console.error(
          "Error updating invoice totals after restore:",
          totalsError
        );
      }
  ...
  const submitEdit = async (updatedInvoice) => {
    try {
      ...
      if (updatedInvoice.isRecurring && updatedInvoice.recurringGroupId) {
        ...
        monthsToUpdate.add(
          `${updatedInvoice.storeID}|${updatedInvoice.targetMonth}|${updatedInvoice.targetYear}`
        );
